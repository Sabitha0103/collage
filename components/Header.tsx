    ...
    <>
      ...
      <div className="fixed top-6 left-6 z-50 mix-blend-difference text-white">
        <Link to="/" className="flex flex-col items-start group">
          <span className="text-2xl font-black tracking-tighter group-hover:opacity-80 transition-opacity">SRIT</span>
        </Link>
      </div>

      {/* Floating Pill Navigation - Bottom Center on Mobile, Top Center on Desktop */}
      <div className={`fixed z-50 transition-all duration-500 ease-out left-1/2 -translate-x-1/2 w-max
          ${scrolled ? 'bottom-8' : 'bottom-8 lg:top-8 lg:bottom-auto'}
      `}>
        <nav className="flex items-center gap-1 p-2 bg-white/90 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.1)] rounded-full text-sm font-bold">
        ...
        {menuOpen && (
          ...
          >
            ...
            <ul className="space-y-4 text-center relative z-10">
              {['About', 'Academics', 'Admissions', 'Research', 'Life at SRIT'].map((item, i) => (
                <li key={item} className="overflow-hidden">
                  ...
                  >
                    <Link
                      to="#"
                      className="block text-5xl md:text-8xl font-black font-heading text-secondary-900 hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-primary-500 hover:to-orange-400 transition-all uppercase leading-none tracking-tighter"
