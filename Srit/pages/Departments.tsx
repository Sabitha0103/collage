        ...
        <div className="bg-white min-h-screen text-secondary-900 pt-32 pb-20">
            <div className="container mx-auto px-4 md:px-12">
                <div className="mb-20 border-b border-gray-100 pb-8">
                    <span className="text-primary-500 font-bold uppercase tracking-widest text-sm mb-2 block">Academics</span>
                ...
                <div className="grid grid-cols-1 gap-px bg-gray-100 border border-gray-100">
                    {departments.map((dept) => (
                        <motion.div
                            key={dept.id}
                            className="bg-white p-12 relative overflow-hidden group hover:bg-secondary-900 hover:text-white transition-colors duration-500"
                            onMouseEnter={() => setHovered(dept.id)}
                            onMouseLeave={() => setHovered(null)}
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
                                <div className="flex items-start gap-6">
                                    <div className="w-16 h-16 rounded-full bg-primary-50 text-primary-500 flex items-center justify-center shrink-0 group-hover:bg-primary-500 group-hover:text-white transition-colors">
                                        <dept.icon size={32} />
                                    </div>
                                    <div>
                                        <h2 className="text-3xl md:text-4xl font-black uppercase mb-3">{dept.name}</h2>
                                        <p className="text-lg text-secondary-500 group-hover:text-white/80 max-w-xl transition-colors">
                                            {dept.desc}
                                        </p>

                                        <div className="mt-8 flex flex-wrap gap-3">
                                            {dept.specializations.map(spec => (
                                                <span key={spec} className="text-xs font-bold uppercase tracking-wider border border-gray-200 px-3 py-1 rounded-full text-secondary-400 group-hover:border-white/20 group-hover:text-white/60 transition-colors">
                                                    {spec}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                <div className="self-end md:self-center">
                                    <Link to={`/department/${dept.id}`} className="w-16 h-16 rounded-full border border-gray-200 flex items-center justify-center group-hover:bg-primary-500 group-hover:border-primary-500 transition-colors">
